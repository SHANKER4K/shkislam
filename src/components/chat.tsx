...
export default function HomePage({ models }: { models: ChatModel[] }) {
  return (
    <PromptInputProvider>
      <Chat models={models} />
    </PromptInputProvider>
...
function Chat({ models }: { models: ChatModel[] }) {
        ...
        try {
          const res = await fetch("/api/chat", {
            ...
            body: JSON.stringify({
              ...
              model_provider: selectedModel?.chef ?? "dahl",
              model_name: selectedModel?.id ?? "moonshotai/Kimi-K2.6",
          ...
          while (true) {
            ...
            for (const raw of events) {
              ...
              if (eventName === "tool") {
                ...
                setMessages((prev) => [
                  ...prev,
                  { type: "tool", role: "assistant", name: data.text, callId: data.tool_call_id, desc },
                ]);
          ...
          setMessages((prev) => [
            ...prev,
            { type: "message", role: "assistant", content: "حدث خطأ في الاتصال. حاول مرة أخرى." },
          ]);
        } finally {
          setStatus("ready");
        }
                ...
                <Fragment key={i}>
                  {msg.type === "tool" ? (
                    <Tool
                      ...
                      toolPart={{
                        type: msg.name,
                        state: msg.output ? "output-available" : "input-streaming",
                    ...
                    <div className={cn(msg.role === "user" && "flex justify-end")}>
                      ...
                      >
                        <MessageContent>
                          <MessageResponse
                            className={cn(
                              msg.role === "assistant" && "text-foreground leading-relaxed",
                            )}
                          >
                            {msg.content}
                          </MessageResponse>
